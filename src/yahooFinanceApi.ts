 ...
 */
export async function fetchLatestListedStocks(): Promise<Array<{
  ticker: string;
  name: string;
  market: 'KR' | 'US';
  exchange?: string;
}>> {
  console.log('최신 상장 종목 목록 가져오는 중...');
  const results: Array<{
    ticker: string;
    name: string;
    market: 'KR' | 'US';
  ...
  // 한국: 최신 상장 종목 검색
  // 주요 인덱스 구성 종목들을 검색하여 누락된 종목 추가
  try {
    // 코스피200, 코스닥150 등 주요 인덱스 관련 검색
    const koreaSearches = ['코스피200', 'KOSPI200', '코스닥150', 'KOSDAQ150', '신규상장', 'IPO'];
    for (const searchTerm of koreaSearches) {
      const matches = await searchYahooSymbol(searchTerm);
      matches.forEach(m => {
        const tickerUpper = m.ticker.toUpperCase();
        if (tickerUpper.includes('.KS') || tickerUpper.includes('.KQ')) {
          const key = tickerUpper.replace(/\.(KS|KQ)$/i, '');
          if (/^[0-9A-Z]{6}$/.test(key) && !seen.has(key)) {
            seen.add(key);
            const isKospi = tickerUpper.includes('.KS') || /^00[0-9]{4}$/.test(key);
            results.push({
              ticker: key,
              name: m.name || key,
              market: 'KR',
    ...
    console.warn('한국 최신 상장 종목 검색 실패:', err);
  }
  
  // 미국: 최신 상장 종목 검색
  // S&P 500, NASDAQ 100 등 주요 인덱스 구성 종목 검색
  try {
    const usSearches = ['S&P 500', 'SP500', 'NASDAQ 100', 'NDX', 'new listing', 'IPO'];
    for (const searchTerm of usSearches) {
      const matches = await searchYahooSymbol(searchTerm);
      matches.forEach(m => {
        if (m.ticker && m.name) {
          const ticker = m.ticker.toUpperCase().replace(/\.(US|)$/i, '');
          if (/^[A-Z]{1,5}$/.test(ticker) && !seen.has(ticker)) {
            seen.add(ticker);
            results.push({
              ticker,
              name: m.name,
              market: 'US',
              exchange: 'NYSE' // 기본값, 실제로는 확인 필요
            });
          }
        }
      });
    ...
    console.warn('미국 최신 상장 종목 검색 실패:', err);
  }
  
  console.log(`최신 상장 종목 ${results.length}개 발견`);
  return results;
}
