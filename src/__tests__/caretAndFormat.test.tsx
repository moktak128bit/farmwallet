...
describe("MoneyField 커서 유지", () => {
  it("금액 중간을 고쳐도 커서가 맨 뒤로 튀지 않는다", () => {
    ...
    // "1,234|,567"에서 커서 위치(5)에 "9"를 입력 → 브라우저가 우선 반영한 원시값은 "1,2349,567"
    fireEvent.change(input, { target: { value: "1,2349,567", selectionStart: 6 } });

    // 숫자만 남겨 다시 콤마를 붙이면 "12,349,567"
    expect(input.value).toBe("12,349,567");
    // 숫자 5개("12349")를 지난 위치 = "12,349|,567"
