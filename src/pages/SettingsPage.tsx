    ...
    <div>
      ...
      {activeTab === "backup" && (
            ...
            onClick={() => {
              ...
              if (Object.keys(preview.countByOldName).length > 0) {
                ...
                for (const [oldName, count] of sorted) {
                  const newName = "" + (NEW_TRANSPORT_SUBS.find((s) =>
                    Object.entries({ ...preview.countByOldName }).some(([o, _c]) => o === oldName)
                  ) ?? "");
                  void newName; // (단순 표시 — 실제 매핑은 미리 알려줌)
        ...
        <div className="card" style={{ borderLeft: "3px solid #ef4444" }}>
          <div className="card-title">⏱ 임시: 이체 &gt; 신용결제이체 → 신용결제 복원</div>
          <p className="hint" style={{ marginBottom: 8 }}>
            실수로 <code>kind=transfer / cat=이체 / sub=신용결제이체</code>로 분류된 항목을
            원래의 <code>kind=expense / cat=신용결제 / sub=신용결제</code>로 되돌립니다.
            <br />
            계좌 잔액 계산이 영향을 받으므로 <strong>실행 전 백업 권장</strong>.
          </p>
          <button
            type="button"
            onClick={() => {
              const targets = data.ledger.filter(
                (l) =>
                  l.kind === "transfer" &&
                  (l.category ?? "") === "이체" &&
                  (l.subCategory ?? "") === "신용결제이체"
              );
              if (targets.length === 0) {
                toast("해당하는 항목이 없습니다 (이체 > 신용결제이체).");
                return;
              }
              const lines: string[] = [
                `신용결제 복원 미리보기`,
                ``,
                `• 변환 대상: ${targets.length}건 (kind=transfer + cat=이체 + sub=신용결제이체)`,
                ``,
                `[샘플 (최대 5건)]`,
              ];
              for (const t of targets.slice(0, 5)) {
                lines.push(
                  `  ${t.date}  ${t.fromAccountId ?? "?"} → ${t.toAccountId ?? "?"}  ${Math.round(t.amount).toLocaleString()}원  "${(t.description ?? "").slice(0, 30)}"`
                );
              }
              lines.push(
                ``,
                ...
                `  category: 이체 → 신용결제`,
                `  subCategory: 신용결제이체 → 신용결제`,
                `  (fromAccountId, toAccountId, amount은 그대로 유지)`,
                ``,
                `이대로 적용하시겠습니까?`
              );
              if (!window.confirm(lines.join("\n"))) return;
              ...
              const nextLedger = data.ledger.map((l) =>
                targetIds.has(l.id)
                  ? {
                      ...l,
              ...
              onChangeData({ ...data, ledger: nextLedger });
              toast.success(`신용결제 복원 완료: ${targets.length}건 변환`);
            }}
            style={{ padding: "8px 16px", fontSize: 13, fontWeight: 600, background: "#ef4444", color: "white", border: "none" }}
          >
            ♻️ 신용결제 복원 실행 (미리보기 후 확인)
          </button>
        </div>
        <div className="card">
