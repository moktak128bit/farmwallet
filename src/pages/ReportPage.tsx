...
export const ReportView: React.FC<Props> = ({ accounts, ledger, trades, prices }) => {
  ...
  const exportCurrentCsv = () => {
    ...
    switch (reportType) {
      ...
      case "investment":
        csv = reportToCSV([
          { 항목: "투자계좌 초기자본", 금액: Math.round(reconciliation.initialCapital) },
          { 항목: "누적 입금", 금액: Math.round(reconciliation.deposits) },
          { 항목: "누적 출금", 금액: Math.round(reconciliation.withdrawals) },
          { 항목: "순투입원금", 금액: Math.round(reconciliation.netContributed) },
          { 항목: "현재 평가액", 금액: Math.round(reconciliation.currentValue) },
          { 항목: "투자 총성과", 금액: Math.round(reconciliation.totalReturn) },
          { 항목: "실현 이익", 금액: Math.round(reconciliation.realizedGain) },
          { 항목: "실현 손실", 금액: Math.round(reconciliation.realizedLoss) },
          { 항목: "실현 손익(순)", 금액: Math.round(reconciliation.realizedPnl) },
          { 항목: "미실현 이익", 금액: Math.round(reconciliation.unrealizedGain) },
          { 항목: "미실현 손실", 금액: Math.round(reconciliation.unrealizedLoss) },
          { 항목: "미실현 손익(순)", 금액: Math.round(reconciliation.unrealizedPnl) },
          { 항목: "배당 수입", 금액: Math.round(reconciliation.dividendIncome) },
          { 항목: "매수 총액", 금액: Math.round(reconciliation.buyVolume) },
          { 항목: "매도 총액", 금액: Math.round(reconciliation.sellVolume) }
        ]);
        filename = "투자_정산";
        break;
    }
