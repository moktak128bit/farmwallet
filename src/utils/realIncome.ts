...
export function classifyIncomeNature(
  sub: string,
  opts?: { salaryKeys?: Set<string>; investIncKeys?: Set<string>; nonRealKeys?: Set<string> }
): IncomeNature {
  const s = (sub || "").trim();
  // 키(자동감지 + 사용자 지정)가 최우선 — 휴리스틱·하드코딩 기본 목록을 덮어쓴다.
  // (기본 salaryKeys/investIncKeys는 정산·용돈 등을 이미 제외하므로 미지정 데이터는 아래 기본 분류로 떨어진다)
  if (opts?.nonRealKeys?.has(s)) return "일시";
  if (opts?.salaryKeys?.has(s)) return "근로";
  if (opts?.investIncKeys?.has(s)) return "패시브";
  // 하드코딩 기본 분류 (사용자 미지정 + 자동감지에도 안 잡힌 카테고리)
  if (s === "대출") return "부채";
...
export function computeRealIncome(
  ...
  for (const l of fInc) {
    ...
    } else if (NON_REAL_INCOME.has(sub) || sub.includes("환불") || (extraNonReal?.has(sub) ?? false)) {
      // 하드코딩 비실질 + 사용자가 설정에서 "비실질"로 지정한 카테고리 → 일시소득(실질수입 제외)
