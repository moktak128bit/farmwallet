...
export function buildDividendGrowth(args: {
  ...
  const currentPrice = currentPriceRow && Number(currentPriceRow.price) > 0 ? Number(currentPriceRow.price) : null;
  ...
  for (const m of months) {
    ...
    while (ti < myTrades.length && myTrades[ti].date <= monthEnd) {
      ...
      if (t.side === "buy") {
        qty += q;
        costBasis += Number(t.totalAmount) || q * (Number(t.price) || 0);
      } else {
    ...
    // 주가: 월말 종가 → 그 달 마지막 거래가 → 시장환경 스냅샷 → (이번 달) 현재 시세
    const price =
      closesByMonth.get(m)?.close ??
      lastTradePriceInMonth ??
      snapPriceByMonth.get(m)?.price ??
      (m === args.currentMonth ? currentPrice : null);
